        ...
        <div className="relative flex flex-col lg:flex-row lg:items-center gap-6">
          ...
          <div className="grid grid-cols-5 gap-2 flex-shrink-0">
            {CALC_DESTACADAS.map((c) => (
              ...
              >
                <span className="text-xl group-hover:scale-110 transition-transform duration-150">{c.emoji}</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
            ...
            <div className="card overflow-hidden border border-slate-100 dark:border-slate-700/60 shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col">
              ...
              <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100 dark:border-slate-700/60 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/10 dark:to-teal-900/10">
                <div className="w-9 h-9 rounded-xl bg-emerald-100 dark:bg-emerald-900/40 flex items-center justify-center text-lg flex-shrink-0 shadow-sm">
                  💰
                </div>
                <div className="flex-1 min-w-0">
              ...
              {ingresosRecientes.length === 0 ? (
                <EmptyState emoji="📭" title="Sin ingresos recientes" subtitle="Los recibos finalizados aparecerán aquí" />
              ) : (
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-semibold text-slate-800 dark:text-slate-200 truncate flex items-center gap-1.5">
                            ...
                            <span className="truncate">{safeStr(rec?.paciente, 'Paciente desconocido')}</span>
                          </p>
            ...
            <div className="card overflow-hidden border border-slate-100 dark:border-slate-700/60 shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col">
              ...
              <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100 dark:border-slate-700/60 bg-gradient-to-r from-violet-50 to-purple-50 dark:from-violet-900/10 dark:to-purple-900/10">
                <div className="w-9 h-9 rounded-xl bg-violet-100 dark:bg-violet-900/40 flex items-center justify-center text-lg flex-shrink-0 shadow-sm">
                  👥
                </div>
                <div className="flex-1 min-w-0">
              ...
              {/* Tabla scorecard */}
              {scorecard.length === 0 ? (
                <EmptyState emoji="👤" title="No hay empleados registrados" subtitle="Los empleados activos aparecerán aquí" />
              ) : (
                          ...
                          >
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                {idx === 0 && <span title="Top 1 — Élite" className="text-base leading-none">🥇</span>}
                                {idx === 1 && <span title="Top 2 — Activo" className="text-base leading-none">🥈</span>}
                                ...
                                {idx > 2 && (
                                  <div className="w-6 h-6 rounded-full bg-gradient-to-br from-blue-400 to-indigo-500 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                                    {safeStr(emp?.empleado_nombre, '?').charAt(0).toUpperCase()}
                                  </div>
                                )}
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            />
            <TareaCard
              icon={totalTareas === 0 ? '😌' : totalTareas <= 3 ? '✅' : '🏃'}
              label="Total de Tareas"
              value={totalTareas}
              sub={totalTareas === 0 ? '¡Día libre!' : totalTareas <= 3 ? 'Carga moderada' : 'Día ocupado'}
      ...
      {!loading && !error && (
        <div className="card p-5 border border-slate-100 dark:border-slate-700/60 shadow-sm hover:shadow-md transition-shadow duration-200">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 flex items-center justify-center text-xl shadow-sm">
              📅
            </div>
            <div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {accesosRapidos.map((a) => (
            ...
            >
              <div className={`w-14 h-14 rounded-2xl ${a.bgClass} flex items-center justify-center text-3xl mb-3 group-hover:scale-110 transition-transform duration-200 shadow-sm relative`}>
                {a.icon}
