...
import { useHospitalizacionesPorExpediente, useCrearHospitalizacion } from '../hooks/useClinico';
...
export default function Hospitalizacion() {
  ...
  const { data: hospitalizaciones = [] } = useHospitalizacionesPorExpediente(expedienteId);
  const { data: empleados = [], isLoading: cargandoEmpleados } = useEmpleados();
  const crearHospitalizacion = useCrearHospitalizacion();
  ...
  const guardar = async () => {
    const ok = await confirm({ title: '¿Guardar hospitalización?', message: 'Confirma que los datos son correctos.', confirmText: 'Sí, guardar', tone: 'warning' });
    if (!ok) return;
    try {
      await crearHospitalizacion.mutateAsync({ expediente_id: expedienteId, ...form, empleados_ids: form.empleados_ids.map(Number) });
      setForm(FORM_EMPTY);
      setMostrarForm(false);
      toast.success('Hospitalización guardada correctamente.');
    } catch (e) {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <PageHeader
        ...
        subtitle={`${hospitalizaciones.length} registro${hospitalizaciones.length !== 1 ? 's' : ''}`}
        action={
      ...
      <FormPanel
        ...
        footer={
          <>
            <button onClick={guardar} disabled={crearHospitalizacion.isPending} className="btn-success disabled:opacity-50">Guardar hospitalización</button>
            <button onClick={() => setMostrarForm(false)} className="btn-secondary">Cancelar</button>
          </>
        }
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <FormField label="Fecha de ingreso" type="date" value={form.fecha_ingreso} onChange={(v) => setForm({ ...form, fecha_ingreso: v })} />
          <FormField label="Fecha de alta" type="date" value={form.fecha_alta} onChange={(v) => setForm({ ...form, fecha_alta: v })} />
          <FormField label="Tipo de alta">
          ...
          {cargandoEmpleados ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {empleados.map((emp) => {
                ...
                return (
                  <label key={emp.id} className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all select-none ${selected ? 'border-amber-400 bg-amber-50 dark:bg-amber-900/20 dark:border-amber-500' : 'border-slate-200 dark:border-slate-700 hover:border-amber-300 dark:hover:border-amber-600 hover:bg-amber-50/50 dark:hover:bg-amber-900/10'}`}>
                    <input type="checkbox" className="sr-only" checked={selected} onChange={() => toggleEmpleado(emp.id)} />
                    <div className={`w-5 h-5 rounded flex items-center justify-center flex-shrink-0 border-2 transition-colors ${selected ? 'bg-amber-500 border-amber-500' : 'border-slate-300 dark:border-slate-600'}`}>
        ...
        {hospitalizaciones.map((h) => (
          <div key={h.id} className="card p-5">
            <div className="flex flex-wrap items-center gap-3 mb-4">
              ...
              <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">Ingreso: {new Date(h.fecha_ingreso).toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
              {h.fecha_alta && <p className="text-sm text-slate-500 dark:text-slate-400">Alta: {new Date(h.fecha_alta).toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric' })}</p>}
              {h.tipo_alta && <span className={h.tipo_alta === 'Fallecimiento' ? 'badge-red ml-auto' : h.tipo_alta === 'Curación' ? 'badge-green ml-auto' : 'badge-blue ml-auto'}>{h.tipo_alta}</span>}
            </div>
            {h.empleados_nombres && (
              <div className="mb-3 flex items-start gap-2">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400"><span className="font-semibold">Personal a cargo:</span> {h.empleados_nombres}</p>
              </div>
            )}

            {/* Hoja de tratamiento por horas (modo tablet) */}
            <button
              onClick={() => navigate(`/tratamiento/${h.id}?paciente=${pacienteId}`)}
              className="mb-3 inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-violet-50 dark:bg-violet-900/20 text-violet-700 dark:text-violet-300 text-xs font-semibold hover:bg-violet-100 dark:hover:bg-violet-900/40 transition-colors"
            >
