...
import { useExpedientes, useCrearExpediente } from '../hooks/useClinico';
...
export default function Expediente() {
  ...
  const { data: paciente, isError: errorPaciente, error: errPac } = usePaciente(pacienteId);
  const { data: expedientes = [], isError: errorExp, error: errExp } = useExpedientes(pacienteId);
  const crearExpediente = useCrearExpediente();

  const [mostrarForm, setMostrarForm] = useState(false);
  const [descargandoPDF, setDescargandoPDF] = useState(false);
  const error = errorPaciente ? mensajeError(errPac, 'Error al cargar el paciente')
    : errorExp ? mensajeError(errExp, 'Error al cargar los expedientes') : null;

  // El paciente seleccionado tiñe el sidebar; se limpia al salir.
  useEffect(() => {
    if (paciente) setSelectedAnimal(paciente);
  }, [paciente, setSelectedAnimal]);
  useEffect(() => () => setSelectedAnimal(null), [setSelectedAnimal]);

  const descargarExpedientePDF = async () => {
    setDescargandoPDF(true);
    try {
      await reportsService.descargarExpediente(pacienteId, paciente?.nombre);
      toast.success('Expediente PDF generado');
    } catch {
      toast.error('No se pudo generar el expediente PDF');
    } finally {
  ...
  const guardar = async () => {
    try {
      await crearExpediente.mutateAsync({ paciente_id: pacienteId });
      setMostrarForm(false);
      toast.success('Expediente creado');
    } catch (err) {
      toast.error(mensajeError(err, 'Error al crear el expediente'));
    }
  };
  ...
  return (
    <div className="animate-fade-in">
      ...
      {paciente && (
        <div className="card p-5 mb-6">
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-wrap gap-2">
            <button onClick={() => navigate(`/vacunas/${pacienteId}`)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-400 text-xs font-semibold hover:bg-teal-100 dark:hover:bg-teal-900/40 transition-colors">
              💉 Ver vacunas
            </button>
            <button onClick={descargarExpedientePDF} disabled={descargandoPDF} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-violet-50 dark:bg-violet-900/20 text-violet-700 dark:text-violet-400 text-xs font-semibold hover:bg-violet-100 dark:hover:bg-violet-900/40 transition-colors disabled:opacity-50">
              {descargandoPDF ? 'Generando…' : '📄 Descargar expediente PDF'}
            </button>
          </div>
        </div>
      )}
      ...
      <FormPanel
        ...
        footer={
          <>
            <button onClick={guardar} disabled={crearExpediente.isPending} className="btn-success disabled:opacity-50">
      ...
      <div className="space-y-3">
        {expedientes.map((e) => (
          <div key={e.id} className="card p-5">
            <div className="mb-4">
              <span className="badge-purple">Expediente #{e.id}</span>
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Apertura: {e.fecha_apertura}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <button onClick={() => navigate(`/consulta/${pacienteId}/${e.id}`)} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-violet-50 dark:bg-violet-900/20 text-violet-700 dark:text-violet-300 text-xs font-semibold hover:bg-violet-100 dark:hover:bg-violet-900/40 transition-colors">
              ...
              <button onClick={() => navigate(`/hospitalizacion/${pacienteId}/${e.id}`)} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300 text-xs font-semibold hover:bg-amber-100 dark:hover:bg-amber-900/40 transition-colors">
              ...
              <button onClick={() => navigate(`/cirugia/${pacienteId}/${e.id}`)} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 text-xs font-semibold hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors">
              ...
              <button onClick={() => navigate(`/recibo/${pacienteId}/${e.id}`)} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 text-xs font-semibold hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-colors">
                🧾 Recibo
              </button>
            </div>
          </div>
