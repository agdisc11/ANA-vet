...
export default function Vacunas() {
  ...
  const [form, setForm] = useState({ nombre: '', fecha_aplicacion: '', proxima_dosis: '', lote: '' });

  const cargar = () => axios.get(`${API}/vacunas/${pacienteId}`).then(r => setVacunas(r.data));

  useEffect(() => { cargar(); }, [pacienteId]);

  const guardar = async () => {
    await axios.post(`${API}/vacunas`, { paciente_id: pacienteId, ...form });
    setForm({ nombre: '', fecha_aplicacion: '', proxima_dosis: '', lote: '' });
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400 uppercase text-xs">
            <tr>{['Vacuna','Aplicacion','Proxima dosis','Lote'].map(h => (
          ...
          <tbody>
            ...
            {vacunas.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-6 text-center text-gray-400">Sin vacunas registradas</td></tr>
            )}
          </tbody>
        </table>
      </div>
