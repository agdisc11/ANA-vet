      ...
      {paciente && (
        <div className="bg-white rounded-xl shadow p-5 mb-6">
          <div className="flex flex-wrap gap-8">
            ...
            <div>
              <p className="text-xs text-gray-400 uppercase">Categorías</p>
              <p className="font-medium">{paciente.especie} / {paciente.sexo}</p>
              {paciente.funcion_zootecnica && <span className="block text-sm text-gray-500">{paciente.funcion_zootecnica}</span>}
            </div>
            <div><p className="text-xs text-gray-400 uppercase">Raza</p><p className="font-medium">{paciente.raza}</p></div>
            {paciente.tatuaje && (
              <div><p className="text-xs text-gray-400 uppercase">Tatuaje</p><p className="font-medium">{paciente.tatuaje}</p></div>
            )}
            {paciente.microchip && (
              <div><p className="text-xs text-gray-400 uppercase">Microchip</p><p className="font-medium">{paciente.microchip}</p></div>
            )}
          </div>
        ...
        {expedientes.map(e => (
          <div key={e.id} className="bg-white rounded-xl shadow p-5">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              <p className="text-xs text-gray-400">Expediente #{e.id}</p>
              <p className="text-sm font-medium text-gray-700">Fecha de apertura: {e.fecha_apertura}</p>
            </div>
