...
const statLabels = {
  tutores: 'Tutores', pacientes: 'Pacientes', consultas: 'Consultas',
  hospitalizaciones: 'Hospitalizaciones', cirugias: 'Cirugías', vacunas: 'Vacunas'
};
...
export default function Reportes() {
  ...
  useEffect(() => {
    API.get('/stats').then(r => setStats(r.data)).catch(console.error).finally(() => setCargando(false));
  }, []);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {reportes.map(r => (
            <button
              ...
              disabled={!!generando}
              className={`${r.color} disabled:opacity-60 disabled:cursor-not-allowed text-white rounded-xl p-4 text-left transition-all duration-150 flex items-start gap-3`}
            >
