...
const links = [
  ...
  { to: '/flujo', label: 'Flujo del día', icon: (
    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
export default function Sidebar({ mobileOpen = false, onClose = () => {}, onOpenSearch = () => {} }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-60 flex-shrink-0 min-h-0 flex flex-col bg-white dark:bg-slate-900 border-r border-slate-100 dark:border-slate-800 shadow-sm transition-transform duration-300 ease-out lg:static lg:z-auto lg:translate-x-0 ${
      ...
      <div className="px-5 py-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
        ...
        <button
          ...
          className="lg:hidden flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-slate-800"
        >
      ...
      {/* Disparador de la búsqueda global (el atajo Ctrl+K vive en AppShell) */}
      <button
        onClick={onOpenSearch}
        className="mx-3 mt-3 flex items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 px-3 py-2 text-sm text-slate-400 dark:text-slate-500 hover:border-violet-300 hover:text-slate-600 dark:hover:border-violet-700 dark:hover:text-slate-300 transition-colors"
      >
        <svg className="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
        </svg>
        <span className="flex-1 text-left">Buscar…</span>
        <kbd className="rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-1.5 py-0.5 font-sans text-[10px] font-semibold">
          Ctrl K
        </kbd>
      </button>
